turtles-own [ pos-x pos-y vel-x vel-y vel ]
...
to setup
  ...
  set delta-t 0.01
  set pos-borde-caja round ( tamanio-caja * max-pxcor / 100 )
  set long-caja 2 * pos-borde-caja - 1

  ask patches with [
    (( abs  pxcor = pos-borde-caja )) and  (abs pycor <= pos-borde-caja) or
    (( abs  pycor = pos-borde-caja )) and  (abs pxcor <= pos-borde-caja)]
  [ set pcolor gray + 2 ]

  create-turtles num-particulas [
    set color yellow
    set pos-x (random-float long-caja) - pos-borde-caja + 0.5
    set pos-y (random-float long-caja) - pos-borde-caja + 0.5
  ...
  actualizar-variables-gas

  set radio-particula 0.05
...
to actualizar-variables-gas
  set volumen long-caja ^ 2
  set temperatura mean [ vel ^ 2 ] of turtles
  set presion mean hist-momento
end
...
to go
  ...
  if colisiones? [
    ask turtles [
      ;; usar in-radius es mucho más eficiente que usar in distance dentro del who...
      let particulas-colisiono sort other turtles in-radius radio-particula with [ who > [ who ] of myself ]
  ...
  if length hist-momento >= 30
  [ set hist-momento but-first hist-momento ]
  set hist-momento lput momento hist-momento
...
to colisionar [ otra-particula ]
  ...
  set vel-x vel-x - ((dvx * drx + dvy * dry) / (drx ^ 2 + dry ^ 2)) * drx
  set vel-y vel-y - ((dvx * drx + dvy * dry) / (drx ^ 2 + dry ^ 2)) * dry
...
to colision-pared
  ...
  if abs proxima-pos-x >= pos-borde-caja - 0.5
  [ set vel-x ( - vel-x )
    set momento momento + (abs 2 * masa * vel ) / long-caja ]

  if abs proxima-pos-y >= pos-borde-caja - 0.5
  [ set vel-y ( - vel-y )
    set momento momento + (abs 2 * masa * vel ) / long-caja ]
...
to colorear-particulas
  ask turtles [
    if vel < ( 1 / 3 ) * max-vel-inicial
    [ set color blue ]
    if vel >= ( 1 / 3 ) * max-vel-inicial and vel < ( 2 / 3 ) * max-vel-inicial
    [ set color yellow ]
    if vel >= ( 2 / 3 ) * max-vel-inicial
    [ set color red ]
  ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot presion"
"pen-1" 1.0 0 -7500403 true "" "plot momento"
...
PENS
"pen-0" 1.0 1 -16777216 true "" "histogram [ vel ] of turtles"

...
globals [
  ...
  ;; variables para generar el espectro
  i-comb
  l-comb
  l-camb
  i-camb
...
to setup
  ...
  set l-comb combinaciones 10 2 1
  set i-comb 0
  set dur 80
  set l-camb n-values (length l-comb) [ i -> dur * i ]
  set i-camb n-values 45 [ i -> (dur * i) + ( dur / 4 ) * 1 ]
  set j-camb n-values 45 [ i -> (dur * i) + ( dur / 4 ) * 3 ]
...
to go
  ...
  ;; generardor de espectro
  if member? ticks i-camb
  [ let comb item i-comb l-comb
    set nivel item 0 comb
    cambiar-nivel
  ]
  if member? ticks j-camb
  [ let comb item i-comb l-comb
    set nivel item 1 comb
    set i-comb i-comb + 1
    cambiar-nivel
  ]
...
to-report combinaciones [ n k inicio ]
  let ns n-values n [ i -> i + inicio ]
  let combs []
  foreach ns [
    i ->
    let nsi n-values ( n - i ) [ j -> i + j + 1 ]
    foreach nsi [
      j ->
      let comb (list i j)
      set combs lput comb combs
    ]
  ]
 report combs
end
